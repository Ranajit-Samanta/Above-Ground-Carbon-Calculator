...
def save_to_excel():
    ...
    data = {
        ...
        "No of trees per 100m": [st.session_state.get("N", "N/A")],
        "Population_Density": [st.session_state.get("A", "N/A")],
        "Height": [st.session_state.get("H", "N/A")],
        "Circumference at breast height": [st.session_state.get("two_pi_r", "N/A")],
        "Circumference at base": [st.session_state.get("two_pi_R", "N/A")],
        ...
        "Above Ground Biomass": [st.session_state.get("above_ground_biomass_results", {}).get("Above Ground Biomass", "N/A")],
        "AGB (Tones per hectare)": [st.session_state.get("above_ground_biomass_results", {}).get("Above Ground Biomass tonnes per hectare", "N/A")],
        "Total Stem Carbon": [st.session_state.get("total_stem_carbon","N/A")],
        "Total Branch Carbon": [st.session_state.get("total_branch_carbon","N/A")],
        "Total Leaf Carbon": [st.session_state.get("total_leaf_carbon","N/A")],
        "Above Ground Carbon": [st.session_state.get("total_carbon", "N/A")],
        "AGC (Tones per hectare)": [st.session_state.get("total_carbon_tonnes_per_hectare", "N/A")],
        "CO2 Equivalent(Tonnes per hectare)": [st.session_state.get("co2_results", {}).get("CO2-Equivalent", "N/A")]
    }
...
st.write("<h1 style='color:white; text-align:center; padding: 20px; background-color: black; font-family: Times New Roman, Times, serif; font-style: italic;'>CARBON CALCULATOR</h1>", unsafe_allow_html=True)
...
with col2:
    st.write("<h1 style='color: purple;'>Field Activities</h3>", unsafe_allow_html=True)
...
if st.button("Calculate Above Ground Biomass:"):
    try:
        ...
        # Store results in session state
        st.session_state.above_ground_biomass_results = {
            "Above Ground Biomass": f"{total_biomass} kg",
            "Above Ground Biomass tonnes per hectare": total_biomass_tones_per_hector
         }
    except:
...
with col3:
    ...
    if st.button("Calculate Carbon"):
        try:
            ...
            st.session_state.carbon_results = {
                "Above Ground Carbon": f"{total_carbon} kg",
                "Above Ground Carbon tonnes per hectare": total_carbon_tonnes_per_hectare
            }
        except:
    ...
    if st.session_state.co2_results:
        ...
        for key, value in st.session_state.co2_results.items():
            st.write(f"{key}: {value} tonnes per hectare")
